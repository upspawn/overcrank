 ...
 *
 * Patches RAF, Date, performance.now, setTimeout, setInterval so the page runs
 * on a virtual clock controlled via window.__virtualTime.advance(ms).
 *
 * This is what makes overcrank work — we control time itself.
 *
 * @file This is a standalone browser script. It runs inside the page context,
 * not in Node. It's read as a string by virtual-clock.ts and injected via
 * page.addInitScript().
 */
  ...
  function flushRAF() {
    ...
    for (var entry of batch) {
      try {
        ...
        console.error('[overcrank] RAF error:', e)
      }
    }
  }
  ...
  function flushTimers() {
    ...
    for (var i = 0; i < snapshot.length; i++) {
      ...
      try {
        ...
        console.error('[overcrank] Timer error:', e)
      }
